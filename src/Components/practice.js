...
// Create a reference to the interactions collection for the current webinar
    const interactionsRef = db
      ...
      .doc(webinarId)
      .collection("interactions");
      // Create a document reference for the new interaction data
    const interactionDocRef = interactionsRef.doc();
...
console.log(interactionDocRef.id)
  
  
    // Set the form data to the document reference
    await interactionDocRef.set(formData);
  
    // Log the document reference ID to the console for future reference
  ...
  function handleEditInteraction(interactionid) {
  }
  ...
  <thead>
    <tr>
    <th><h6>Interaction Type</h6></th>
      <th><h6>Start Time</h6></th>
      <th><h6>End Time</h6></th>
      <th><h6>Question / Title / Sender Message</h6></th>
      <th><h6>Answers / Subtitle / Button Text</h6></th>
      <th><h6>Button Link</h6></th>
      <th><h6>Show Result</h6></th>
      <th><h6>Custom Message</h6></th>
      <th><h6>Admin</h6></th>
      <th><h6>Edit</h6></th>
      <th><h6>Delete</h6></th>
    </tr>
  ...
  <tbody>
    {interactionsData.map((interaction, index) => (
      <tr key={index}>
        ...
        <td>{interaction.startpollTime || interaction.startTimeCTA || interaction.startMessageTime}</td>
        <td>{interaction.endpollTime || interaction.endTimeCTA || interaction.endMessageTime}</td>
        <td>{interaction.question || interaction.titleCTA || interaction.senderMessage}</td>
        <td>{interaction.answers || interaction.subtitleCTA || interaction.buttonText}</td>
        ...
        <td>
  <button className="edit-button" onClick={() => handleEditInteraction(interaction.interactionid)}>
    <FontAwesomeIcon icon={faEdit} /> <span>Edit</span>
  </button>
</td>
<td>
  <button className="delete-button" onClick={() => handleDeleteInteraction(interaction.interactionid)}>
...
const handleInteractionSubmit = async (e) => {
  ...
  if (interactionType === "polls") {
    ...
    if (!startTimeCTA || !endTimeCTA || !titleCTA || !subtitleCTA || !buttonText || !buttonLink) {
      isFormValid = false;
      alert("Please fill in all the required fields.");
    }
  } else if (interactionType === "chats") {
    if (!startMessageTime || !endMessageTime || !senderName || !senderMessage) {
      isFormValid = false;
      alert("Please fill in all the required fields.");
    }
  } else {
